...
def get_category_offers(self, category, item_count: int = 10, item_page: int = 1,
                        min_saving_percent: int = None, exclude_zero_offers: int = 0, keywords: str = None, ):
    try:
    ...
    while redis_manager.redis_db.llen(key_cache_db) < MAX_ITEM_COUNT_OFFER * MAX_ITEM_PAGE_OFFER:
        try:
            product_lists = []
            ...
            redis_manager.redis_db.lpush(key_cache_db, *product_lists)
            redis_manager.redis_db.expire(key_cache_db, CATEGORY_REFRESH_TIMEOUT_SECONDS)
            self.update_state(state="PROGRESS",
                              meta=get_meta(page=page_download, total_element=len(product_lists),
                                            category=key_cache_db))
            page_download += 1
            ...
            ttl_category = redis_manager.redis_db.ttl(key_cache_db)
            ...
            redis_manager.redis_db.set(completed_key, 0)
            ttl_category = redis_manager.redis_db.ttl(page_download)
            ttl_category = CATEGORY_REFRESH_TIMEOUT_SECONDS if ttl_category < 0 else ttl_category
    ...
    return get_final_meta(total_time_s=total_time, total_element=total_element, category=key_cache_db)
